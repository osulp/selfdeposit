...
// ||	<script language="JavaScript" src="http://resolver.orbiscascade.org/pt/worldcat.js"></script> 	||
...
/*-----------------------------------------------------------------------------
 ...
 Changes :
    02/05/13    KEF     Re-worked the parsing of the query parameters for
                        searches that could involve ISBN/ISSN values.  It was
                        a mess.  I replaced a lot of substring operations with
                        regex replacement, which is much cleaner.  Before the
                        fix, many ISSN searches were not being matched.
-----------------------------------------------------------------------------*/
...
document.write("\<script src='//ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js' type='text/javascript'>\<\/script>");
...
if (str.indexOf("griffin.wsu.edu") > -1) {
	...
	url = "http://pugetsound.worldcat.org/search?q=";
...
function BibRecord2() { //----------- If we are in a bib.record details page -------------------------
	...
	if (IsStaff) {
		...
		v_css.type = 'text/css';
		v_css.href = "http://resolver.orbiscascade.org/pt/custom_dialog.css";
		...
		function createDialog() {
			...
			try {
				...
				dialogMessage.innerHTML = "<b>Do you want to send your original search to Summit or to access just this record?</b> <br>"
...
//	sqf = sqf.replace("+", " ");
*/
	// Extract out the search term from the query string.
	var sqf = str.replace(/^(.*?)SEARCH=(.*?)&(.*?)$/g, '$2');
...
*/
	var sqindex = str.replace(/^(.*?)search\/(\w)\?SEARCH=(.*?)$/g, '$2');
...
*/
	// Extract out the search term from the query string.
	var sqf = str.replace(/^(.*?)searcharg=(.*?)&(.*?)$/g, '$2');
...
*/
	// Extract out the search type from the query string.
	var sqindex = str.replace(/^(.*?)searchtype=(\w)(.*?)&(.*?)$/g, '$2');
...
function readit2() {
	...
	if (!sfarg) {
		var fmx = document.getElementsByName("q"); //--- use named form field to get searcharg ---
		if (fmx[0]) {
			var fmxval = fmx[0].value;
		} else {
			var fmxval = sqf;
	...
	if ((document.searchtool) || (fmxval) || (sfarg)) {
		if ((document.searchtool) && (!sfarg)) {
			var sfarg = document.searchtool.searcharg.value;
		} //--- in case we didn't get searcharg above ---
		else {
			if (fmxval) {
				var sfarg = fmxval;
			} else {
		...
		if ((d1) || (d2) || (wclimit) || (wclang) || ((str.indexOf("limit?") > -1 || str.indexOf("&m=") > -1 || str.indexOf("&l=") > -1 || str.indexOf("&Da=") > -1 || str.indexOf("&Db=") > -1) && (!(str.indexOf("frameset&FF=") > -1) && !(str.indexOf("public&FF=") > -1)))) {
...

			{
				...
				if (((d1) || (d2)) || ((str.indexOf("&Da=") > -1 || (str.indexOf("&Db=") > -1 || (str.indexOf("&Ya=") > -1 || (str.indexOf("&Yb=") > -1)))))) {
					if (!d1) {
						d1 = "1"
					} //--- Supply after date for WC if user did not specify ---
					if (!d2) {
						d2 = "2200"
					} //--- Supply before date for WC if user did not specify ---
					if (str.indexOf("&Da=") > -1) {
...
if (str.indexOf("library.seattleu.edu") > -1) { //-- Seattle U does not use an image.
	...
	if (!document.getElementById('SummitBtn')) {
		document.write("<a href='javascript:readit2();'><img src='" + Protoc + Summit_Img + "' border=0 width=10% id=SummitBtn></a>");
	}
}
...
/*if (str.indexOf("library.seattleu.edu") > -1) { //-- Seattle U does not use an image.
	...
	if (!document.getElementById('SummitBtn')) {
		document.write("<a class="ng-scope" target="_blank" ng-if="link.type == 'custom'" ng-href=");
		document.write("<a href='javascript:readit2();'><img src='" + Protoc + Summit_Img + "' border=0 id=SummitBtn></a>");
		document.write("<span class="ng-scope ng-binding" ng-bind-html-unsafe="link.label" ng-if="!link.image">Try Summit</span></a>");
		
		if (document.getElementByClassName('ng-scope ng-binding')) {
			var link = document.getElementByClassName('ng-scope ng-binding');
			link.onclick = readit2();
		}
	}
}
*/
